...
export default function LoginScreen() {
  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      setError(msg.includes('Failed to fetch') || msg.includes('Network') ? 'Não foi possível conectar ao servidor.' : msg);
    } finally {
      setLoading(false);
    }
  };
  ...
  const cardInner = (
    <View style={styles.cardInner}>
      ...
      <Text style={styles.subtitle}>
        {backendConfigured ? 'Autenticação segura via servidor.' : 'Modo demonstração — nenhuma credencial hardcoded. Conecte um backend para segurança real.'}
      </Text>

      {!backendConfigured && (
        <View style={styles.infoBox}>
          <Ionicons name="shield-checkmark-outline" size={14} color="#a0a0a0" />
          <Text style={styles.infoText}>Qualquer usuário/senha não vazios liberam o acesso neste modo. Configure EXPO_PUBLIC_AUTH_URL.</Text>
        </View>
      )}
      ...
      <View style={styles.field}>
        ...
        <View style={styles.inputWrap}>
          ...
          />
          <Pressable onPress={() => setShowPass((v) => !v)} hitSlop={10} accessibilityRole="button" accessibilityLabel={showPass ? 'Ocultar senha' : 'Mostrar senha'}>
            <Ionicons name={showPass ? 'eye-off-outline' : 'eye-outline'} size={18} color="#a0a0a0" />
          </Pressable>
        </View>
      </View>
      ...
      <Pressable
        onPress={onSubmit}
        disabled={loading}
        style={({ pressed }) => [styles.btn, pressed && { opacity: 0.9, transform: [{ scale: 0.99 }] }, loading && { opacity: 0.7 }]}
      ...
      <Text style={styles.foot}>Sessão válida por 24 horas. Toque em Sair no cabeçalho para encerrar.</Text>
    </View>
  );
...
function createStyles(colors: ReturnType<typeof useSchoolColors>) {
  return StyleSheet.create({
    ...
    field: { gap: 6 },
    label: { fontSize: 12, fontWeight: '700', color: '#a0a0a0', letterSpacing: 0.3, textTransform: 'uppercase' },
    inputWrap: {
      flexDirection: 'row',
      alignItems: 'center',
      gap: 10,
