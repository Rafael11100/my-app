...
function SensorMiniRow({
  sensor,
}: {
  sensor: import("@/context/alerts-context").Sensor;
}) {
  ...
  const scale = pulse.interpolate({
    inputRange: [0, 1],
    outputRange: [1, isSmoke ? 1.12 : 1.06],
  });
  const opacity = pulse.interpolate({
    inputRange: [0, 1],
    outputRange: [1, isSmoke ? 0.85 : 0.92],
  });
  ...
  return (
    <View
      ...
      style={[
        styles.miniRow,
        {
          borderColor: sensor.active ? accent + "55" : "rgba(160,160,160,0.12)",
    ...
    >
      <Animated.View
        style={[
          styles.miniIcon,
          {
            backgroundColor: sensor.active ? accent : "rgba(255,255,255,0.08)",
            transform: sensor.active ? [{ scale }] : undefined,
            opacity: sensor.active ? opacity : 1,
          },
        ]}
      >
        <Ionicons
...
export default function HomeScreen() {
  ...
  const totalOk =
    smokeSensors.filter((s) => !s.active).length +
    motionSensors.filter((s) => !s.active).length;
  const overviewTitle = hasActive
    ? smokeActive
      ? "Alerta de fumaça"
      : "Movimento detectado"
    : "Sistema normal";
    ...
    <View style={[styles.screen, { backgroundColor: "#1f1f1f" }]}>
      ...
      >
        ...
        <View style={styles.overviewWrap}>
          {Platform.OS === "web" ? (
            <View
              style={[styles.overviewCard, styles.cardWeb]}
          ...
          <View style={styles.groupHeader}>
            ...
            {activeSmoke.length > 0 && (
              <View style={styles.groupBadgeAlert}>
                <Text style={styles.groupBadgeAlertText}>
                  {activeSmoke.length} alerta
                </Text>
              </View>
            )}
          </View>
        ...
        <View style={styles.group}>
          <View style={styles.groupHeader}>
            ...
            {activeMotion.length > 0 && (
              ...
              >
                <Text
                  style={[styles.groupBadgeAlertText, { color: "#60A5FA" }]}
                >
                  {activeMotion.length} detectado
                </Text>
              </View>
            )}
          </View>
          ...
          <View style={styles.actionsGrid}>
            ...
            >
              ...
              <View style={styles.actionTextWrap}>
                ...
                <Text style={styles.actionSub}>
                  5 sensores · Banheiros e áreas técnicas
                </Text>
              </View>
              <View
                style={[
            ...
            >
              ...
              <View style={styles.actionTextWrap}>
                ...
                <Text style={styles.actionSub}>
                  5 sensores · Corredores e acessos
                </Text>
              </View>
              <View
                style={[
...
const styles = StyleSheet.create({
  ...
  actionTextWrap: { flex: 1, minWidth: 0, gap: 2 },
  actionTitle: {
    fontSize: 13,
    fontWeight: "800",
    color: "#F2F2F2",
  ...
  actionSub: { fontSize: 11, color: "#7a7a7a", flexShrink: 1 },
