...
function SensorDetailCard({
  ...
  const scale = pulse.interpolate({
    inputRange: [0, 1],
    outputRange: [1, isSmoke ? 1.14 : 1.06],
  });
  const glowOpacity = glow.interpolate({
    inputRange: [0, 1],
    outputRange: [0, isSmoke ? 0.18 : 0.12],
  });

  const statusLabel = sensor.active
    ? isSmoke
      ? "ALERTA DE FUMAÇA"
      : "MOVIMENTO DETECTADO"
    : "Normal";
  const statusIcon = sensor.active
    ? isSmoke
      ? "warning-outline"
      : "walk-outline"
        ...
        <Animated.View
          style={[
            styles.iconCircle,
            {
              ...
              borderColor: sensor.active
                ? accent + "40"
                : "rgba(160,160,160,0.12)",
              transform: sensor.active ? [{ scale }] : undefined,
            },
          ]}
        >
          <Ionicons
      ...
      <View style={styles.cardActions}>
        <Pressable
          ...
          style={({ pressed }) => [
            styles.toggleBtn,
            {
              backgroundColor: sensor.active ? "#3a3a3a" : accent,
              borderColor: sensor.active
        ...
        >
          <Ionicons
            name={sensor.active ? "close-circle-outline" : "power-outline"}
            size={16}
            color="#fff"
          />
          <Text style={styles.toggleText}>
            {sensor.active ? "Desativar" : "Ativar"}
          </Text>
        </Pressable>
        <Pressable
          onPress={() =>
        ...
        >
          ...
          <Ionicons name="chevron-forward-outline" size={12} color="#a0a0a0" />
        </Pressable>
      </View>
    </View>
  );
...
export default function SensorScreen({ kind, title, subtitle }: Props) {
  ...
  const { smokeSensors, motionSensors, activateSensor, deactivateSensor } =
    useAlerts();
  ...
  const overlayColor = glow.interpolate({
    ...
    outputRange: [
      "rgba(0,0,0,0)",
      isSmoke ? "rgba(239,68,68,0.06)" : "rgba(59,130,246,0.06)",
    ],
  });
    ...
    <View style={[styles.screen, { backgroundColor: "#1f1f1f" }]}>
      ...
      >
        ...
        <View style={styles.headerBlock}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.headerSub}>
              {subtitle} · {sensors.length} sensores
            </Text>
          </View>
          <View
            style={[
          ...
          >
            <Text
              style={[
                ...
                { color: activeCount ? accent : "#a0a0a0" },
              ]}
            >
        ...
        {isSmoke && activeCount > 0 && (
          ...
          >
            ...
            <Text style={styles.bannerText}>
              Possível situação de fumaça — verifique o local imediatamente.
            </Text>
          </View>
        )}
        {!isSmoke && activeCount > 0 && (
          ...
          >
            <Ionicons name="walk-outline" size={16} color="#60A5FA" />
            <Text style={[styles.bannerText, { color: "#93C5FD" }]}>
              Movimento detectado — presença identificada.
            </Text>
          </View>
        )}
        ...
        <View style={styles.list}>
          {sensors.map((s) => (
            <SensorDetailCard
              ...
              sensor={s}
              onToggle={() =>
                s.active ? deactivateSensor(s.id) : activateSensor(s.id)
              }
            />
          ))}
        </View>
        ...
        <Text style={styles.hint}>
          {isSmoke
            ? "Fumaça exige resposta imediata."
            : "Movimento é informativo — não indica emergência de fumaça."}{" "}
          Toque em Ativar para simular detecção (10s fumaça · 2s presença).
        </Text>
      </ScrollView>
    </View>
  );
